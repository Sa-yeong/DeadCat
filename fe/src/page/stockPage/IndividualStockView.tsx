...
export function IndividualStockView(){
    ...
    return <>
                    ...
                    <span className='left'>
                        ...
                        <span className='content'>
                            <span className='up'>
                                ...
                                <span onClick={()=>toggleHeart(currentStock?.is_favorite ?? false)}
                            ...
                            <span className='down'>
                                <span style={{color:(currentStock?.change_rate > 0)? 'tomato': ((currentStock?.change_rate==0)?'gray':'skyblue')}}>
                                    {currentStock?.current_price?.toLocaleString()}</span>
                                <span style={{color:(currentStock?.change_rate > 0)? 'tomato': ((currentStock?.change_rate==0)?'gray':'skyblue')}}>
        ...
        <LoginModal isOpen={isOpen} onClose={()=>setIsOpen(false)} current_page={window.location.pathname} />
    </>;
}
...
function AllStockList({stock_code, is_like, img, name, rate, is_event, setCurrentStock, currentTab, toggleHeart}) {
    ...
    return <Link to={`/stocks/individual/${stock_code}/${currentTab}`} className="row" onClick={()=>setCurrentStock(stock_code)}>
        <span onClick={(e) => toggleHeartAll(e, is_like)}>
            { is_like? <IoHeart color='red' /> : <IoHeartOutline /> }
        </span>
